...
static void intToByte(int i) {
    zb_abyte[0] = (byte) (0xff & i);
    zb_abyte[1] = (byte) ((0xff00 & i) >> 8);
    zb_abyte[2] = (byte) ((0xff0000 & i) >> 16);
    zb_abyte[3] = (byte) ((0xff000000 & i) >> 24);
    ZIB_DBG("abyte:0x%x,0x%x,0x%x,0x%x", zb_abyte[0], zb_abyte[1], zb_abyte[2], zb_abyte[3]);
}
...
static void set_adapter_discoveryMode() {
    ...
    prop.len = 4;
    intToByte(scan_mode);
    ZIB_DBG("set_adapter_discoveryMode:0x%x,0x%x,0x%x,0x%x", zb_abyte[0], zb_abyte[1], zb_abyte[2], zb_abyte[3]);
    prop.val = zb_abyte;
...
static void
ssp_request_cb(bt_bdaddr_t *remote_bd_addr, bt_bdname_t *bd_name, uint32_t cod, bt_ssp_variant_t pairing_variant,
               uint32_t pass_key) {
...
void bdt_le_test_mode(char *p) {
    ...
    switch (cmd) {
        ...
        case 0x2: /* TX TEST */
            ...
            arg2 = get_int(&p, -1);
            arg3 = get_int(&p, -1);
            if ((arg1 < 0) || (arg2 < 0) || (arg3 < 0)) ZIB_DBG("%s Invalid arguments", __FUNCTION__);
            buf[0] = arg1;
            buf[1] = arg2;
            buf[2] = arg3;
        ...
        case 0x3: /* END TEST */
            ...
            break;
        default: ZIB_DBG("Unsupported command");
            return;
            break;
    }
    if (status != BT_STATUS_SUCCESS) {
...
void uuid2str(const bt_uuid_t bdaddr) {
    ZIB_DBG("%X:%X:%X:%X:%X:%X:%X:%X:%X:%X:%X:%X:%X:%X:%X:%X",
            bdaddr.uu[0], bdaddr.uu[1], bdaddr.uu[2], bdaddr.uu[3], bdaddr.uu[4], bdaddr.uu[5], bdaddr.uu[6],
            bdaddr.uu[7],
            bdaddr.uu[8], bdaddr.uu[9], bdaddr.uu[10], bdaddr.uu[11], bdaddr.uu[12], bdaddr.uu[13], bdaddr.uu[14],
            bdaddr.uu[15]);
}
...
void btgattc_register_for_notification_cb(int conn_id, int registered, int status,
    ...
    ZIB_DBG("%s::%d, registered = %d, conn_id = %d, status = %d is_primary = %d, id.inst_id = %d",
